...
export default function Step4({ onSkip }: { onSkip?: () => void } = {}) {
  ...
  const addCollaborator = () => {
    ...
    setTimeout(() => {
      const el = document.getElementById(`collab-name-${id}`) as HTMLInputElement | null;
      el?.focus();
      el?.select();
    }, 60);
  };
        ...
        <div className="px-4 py-4 bg-gradient-to-r from-slate-50 to-white border border-neutral-200 rounded-xl max-w-[700px] my-4 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground max-w-xl">
              Administradores têm acesso total. Profissionais podem gerenciar
              seus horários e atendimentos. Assistentes têm acesso limitado.
            </p>
          </div>
        </div>
      ...
      <div className="flex gap-3 mt-4 items-center border-t py-4">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="font-medium group-hover:text-sky-300">Adicionar</span>
          </Button>
